...
class Scrape(object):
    def __init__(self):
        ...
        self.url = input("Enter hqdiesel Album Url\ne.g.\thttps://hqdiesel.net/thumbnails.php?album=\t\n\t:")
        self.page_title = ""
        self.total_pages = 0
    ...
    def get_images(self):
        with self.session as s:
            ...
            thread_map(self._download_images, range(0, len(self.image_urls)))
    ...
    def get_image_urls(self, s, url, page_count=False, page_title=False):
        ...
        if page_title:
            # temp=bs.title.string.split("-")
            self.page_title =bs.title.string.split(" -")[0]
            self.file_path = self.package_dir
    ...
    def _download_images(self, pos):
        with self.session as s:
            try:
                ...
                r = s.get(f"https://www.hqdiesel.net/{image_src}", stream=True)
                r.raw.decode_content = True
                pname=image_src.split("/")

                # with open(f"{self.file_path}\\_{pos}.jpg", 'wb+') as f:
              #  with open(f"{self.file_path}\\{self.page_title}_{pos}.jpg", 'wb+') as f:
                with open(f"{self.file_path}\\{pname[-1]}", 'wb+') as f:
